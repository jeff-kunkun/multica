...
#   $env:MULTICA_MODE="local"; irm https://raw.githubusercontent.com/jeff-kunkun/multica/kun/scripts/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
# Mirrors scripts/install.sh: this fork ships its own CLI archives, so the
# default source is the fork, not upstream (DENE-420). Set MULTICA_REPO to
# "multica-ai/multica" to install the upstream build instead.
$RepoSlug      = if ($env:MULTICA_REPO) { $env:MULTICA_REPO } else { "jeff-kunkun/multica" }
$RepoBranch    = if ($env:MULTICA_REPO_BRANCH) { $env:MULTICA_REPO_BRANCH } else { "kun" }
...
function Start-DefaultInstall {
    ...
    Write-Host "     `$env:MULTICA_MODE=`"with-server`"; irm $RepoRawUrl/scripts/install.ps1 | iex"
    Write-Host ""
}
...
function Start-LocalInstall {
    ...
    Write-Host "     `$env:MULTICA_MODE=`"stop`"; irm $RepoRawUrl/scripts/install.ps1 | iex"
    Write-Host ""
}
