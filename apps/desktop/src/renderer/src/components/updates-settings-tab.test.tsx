...
describe("UpdatesSettingsTab", () => {
  ...
  it("loads the persisted release channel and switches it from the picker", async () => {
    bridge.fns.getPreferences.mockResolvedValue({
      automaticUpdates: true,
      releaseChannel: "test",
    });
    ...
    await waitFor(() => expect(channel).toHaveValue("test"));
    ...
    // Switching channels is a preference, not a reinstall. Copy that hints
    // otherwise would be a lie about what the button does.
  ...
  it("keeps the old channel selected when saving it fails", async () => {
    bridge.fns.getPreferences.mockResolvedValue({
      automaticUpdates: true,
      releaseChannel: "stable",
    });
    bridge.fns.setReleaseChannel.mockRejectedValue(new Error("EACCES"));
    render(<UpdatesSettingsTab />);
