...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("  | Top-level issue delivered, awaiting acceptance | `--outcome in_review --evidence-file ./close.md` (needs a linked open/merged PR — a docs or research ticket with no code says why with `--no-code <reason>`, otherwise the close is refused; an empty reviewer slot is filled with a different-family acceptance seat in the same call, then routing hands it over; add `--needs-human <member>` only when a named person must decide) |\n")
