...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("- You delivered what the issue itself asks for and it awaits acceptance → `in_review`. This acceptance state belongs only to a top-level issue: its reviewer checks the parent together with the complete child-issue tree. A sub-issue is execution-only — do not fill or trigger a reviewer for it, and do not move it to `in_review`; finish the child through the close protocol so the parent barrier can account for it. When acceptance passes, the acceptance seat merges and sets `done` in that same turn if the checks this change owns are green and the ticket does not explicitly name a person and a decision still waiting on them (`close.conclusion=awaiting_human`). A check already red on the base branch is not that wait, and neither is a routing note that says 需要人拍板. Do not leave a passed ticket in `in_review` for a person to click merge.\n")
	...
	b.WriteString("  | --- | --- |\n")
	b.WriteString("  | Sub-issue finished, or a top-level issue that needs no acceptance | `--outcome done --evidence-file ./close.md` |\n")
	b.WriteString("  | Top-level issue delivered, awaiting acceptance | `--outcome in_review --evidence-file ./close.md` (routing hands it to the acceptance seat; add `--needs-human <member>` only when a named person must decide) |\n")
