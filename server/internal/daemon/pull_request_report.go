...
type localPR struct {
	...
	Title, State, URL, HeadRefName, HeadRefOid string
	IsDraft                                    bool
	MergedAt                                   *time.Time
}
...
func (d *Daemon) reportLocalPullRequests(ctx context.Context, task Task, result TaskResult) error {
	...
	remote := exec.CommandContext(ctx, "git", "-C", dir, "config", "--get", "remote.origin.url")
	b, err := remote.Output()
	if err != nil {
		return err
	}
	...
	reports := make([]map[string]any, 0, len(prs))
	for _, p := range prs {
		reports = append(reports, map[string]any{"owner": parts[len(parts)-2], "repo": parts[len(parts)-1], "number": p.Number, "title": p.Title, "state": strings.ToLower(p.State), "url": p.URL, "branch": p.HeadRefName, "sha": p.HeadRefOid, "merged_at": p.MergedAt})
	}
