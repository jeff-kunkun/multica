...
func (h *Handler) CreateAgent(w http.ResponseWriter, r *http.Request) {
	...
	if !inheritRuntime {
		...
		// An effort with no pinned model is not storable for runtimes that
		// resolve their own default model out of sight — it would save cleanly,
		// show as set, and then run at a different level. Reject it here instead
		// of letting the daemon drop it silently at launch (MUL-7412).
