...
§3.3 / §4.2 的结论「不加第三个 policy key」被 DENE-421 的终局决策推翻：策略表加了第三个 key `frontend`，前端拷问升成独立策略，入口那颗策略药丸就是切换处。它放弃的是「一场对齐一条 policy 记录」能覆盖全部讨论内容的假设 —— 换来的是用户可以显式点名这件事，而不是让前端 grill 去跟需求拷问抢回合。

本页仍是该策略正文的直接来源：

- §2 的载体能力核查 —— 能写工作目录、能把 HTML 作为回复附件交出来、附件在对齐页里是内联 iframe。
- §3.2 的收成物 —— `multica attachment upload` 产出、贴在那轮回复上的单个 HTML 文件；「没有可打开的收成物 = 这一轮没完成」在策略里逐字保留。
- §4 的方法 —— 一屏一档为默认、五方向用于显式比较、desktop/phone 两种宽度、loading/empty/error 三态横切、一次只问一个选择。
- §8 的 Q2 / Q4 —— 两种考法的默认值与「一场对齐最多两屏」。

只有 §3.1 的触发方式变了：它原本是「载体提议 + 用户一键」，现在用户是显式选了这一档，所以策略正文改成直接开工、不再重复提议。
