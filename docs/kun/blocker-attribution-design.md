...
> DENE-520 更新：本页 §1.1 / §2.1 / §5 引用的 Stage 4 停滞看门狗（`stagnation_watchdog*.go`、`watchdogStaleAfter`、四扫描）已整层摘除，`stage_wakeup_failure` 表不再有新写入。涉及它的行只作历史记录；`wake_missed` 之类推导态的阈值以 `close.at` + 前端推导为准，不再与服务端扫描对齐。
