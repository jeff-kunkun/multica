...
- **附件走分片续传，断了不用从零开始**（DENE-443）。附件分组里一个 6 MB 的 blob 在这条上行链路上要传两分多钟，超过 Cloudflare 的 100 秒代理超时，所以整块传必然被截断。目标实例在 `/health` 里 advertise `attachment_chunk_max_bytes` 之后，CLI 会先把整个附件组问一遍「你已经有哪些、每个收到多少字节」，然后只补缺的部分，每片 2 MB 量级按实测速率自适应，全部到齐后校验 sha256 再落库。**上传途中被 kill 或断网，重跑同一个包即可**：已落库的附件不再发，半截的分片从断点接着传。目标太旧、不带这个能力时退回整块上传，单个 blob 超过 4 MB 会直接报 `target_unsupported` 并说明原因，不会传到一半被代理切断。
- 附件进度按**字节**报（`attachments_bytes_uploaded` / `attachments_bytes_total`），不是按附件个数：370 个附件里大小差两个数量级，按个数的进度条会停在大附件上不动。
...
1. 打开 Desktop（此刻连的是官方云）→ 进入要导出的工作区 → 设置 →「配置导出/导入」→ 卡片「跨环境迁移（含对话）」→ 看导出源是不是官方云的那个工作区 → 勾选要带的分组（见下）→ 点【一键导出】→ 选保存位置 → 等进度走完 → 记下 zip 路径。
   ...
   进度卡会写明当前在导哪一组（配置 / 对话 / 任务）和一条进度条。**导出跑在主进程里，切到别的设置页不会中断它**：回到这张卡会自动接回正在跑的那次导出，跑完了则直接显示「已保存为 …」。同理，导出期间再点【一键导出】不会另起一次，卡片会把正在跑的那次显示出来。
