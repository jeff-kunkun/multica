...
跨环境迁移的语义是复刻同一套环境，所以默认往「迁过去就能用」靠。三个选项在 CLI 与 Desktop 卡片上含义一致，默认值也一致，并且同时作用于预演和正式导入：预演报告里的数字就是按当前开关算出来的。
...
Desktop 的「跨环境迁移」卡片上有同样三个勾选项，默认值与上表一致；预览报告会写明「自动化：导入 N 条，其中 M 条已暂停」，勾上激活再应用一次，它们会立刻开始触发。
...
卡片上「导入后激活自动化」「应用工作区设置」「采用 issue 前缀」三个勾选项的默认值与上文的 CLI 选项表一致；只有第一条会影响「自动化」是否马上开始跑，预演报告里会直接写明导入几条、几条已暂停。
