...
按钮由同 Stage 的姊妹票实现。卡片只在 Desktop 下出现；网页版没有这个按钮（官方云没有服务端导出端点，网页端也做不了本地打包），网页版要走上面的命令行。

1. 打开 Desktop → 进入要导出的工作区 → 设置 →「配置导出/导入」→ 卡片「跨环境迁移（含对话）」→ 点【一键导出】→ 选保存位置 → 等进度走完 → 记下 zip 路径。
2. 用 Desktop 右上角/设备设置里的服务器切换，切到目标自建实例（`kun` 版本）→ 进入目标工作区 → 同一张卡片 → 点【一键导入】→ 选那个 zip → 先看预演报告 → 确认 → 等导入完成。
3. 读导入报告：`secrets_to_fill`（要你去哪儿补哪些密钥）、`runtimes_to_bind`（要在哪台机器上绑哪个运行时）、`export_gaps`（源端哪些分组没导出、为什么）。
