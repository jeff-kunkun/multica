...
10. **macOS 自动更新要求所有包用同一张证书签。** ad-hoc 签名的 designated requirement 锁在这一份二进制的哈希上，下一版必然对不上。证书只生成一次；换一张，已经装出去的客户端就再也收不到更新。私钥、`.p12`、密码不进仓库、不进票、不进评论。

## macOS 自签证书

owner 不使用 Apple Developer ID。自动更新靠一张长期自签的 Code Signing 证书：本机生成一次，导出 `.p12`，由 owner 自己写进仓库 secret，GitHub Actions 用同一张签每一个 mac 包。只在自己电脑上签、CI 仍打 ad-hoc，包和包之间还是对不上。

没有 `CSC_LINK` 时，workflow 保持原来的 ad-hoc 行为，缺 secret 不会让这条流水线变红。Windows / Linux 不读这张证书，继续不签名。
