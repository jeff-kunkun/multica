...
/**
 * `--gemini_dir` is the agy account binding, and the accounts tab now owns it.
 * It stays in `custom_args` (and is saved untouched from here) but is kept out
 * of the visible list, so this tab cannot become a second, unvalidated entry
 * point for choosing an account.
 */
...
export function CustomArgsTab({
  ...
  useEffect(() => {
    onDirtyChange?.(argsDirty);
  }, [argsDirty, onDirtyChange]);
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="max-w-2xl text-pretty text-caption leading-5 text-muted-foreground">
      ...
      <div className="flex items-center justify-end gap-3 pt-1">
        ...
        <Button
          onClick={handleSave}
          disabled={!argsDirty || saving || editor !== null}
          size="sm"
        >
          {saving ? (
            <Loader2
