export type AgyAccountSlot = "account1" | "account2" | "account3" | "custom";
...
export function inferHomeDirFromGeminiPath(path: string): string | null {
  ...
  const match = normalized.match(/^(.*)[/\\]\.gemini(?:-account[23])?$/);
  return match?.[1] || null;
}
...
export function detectAgyAccountSlot(profile: string): AgyAccountSlot {
  ...
  if (trimmed === `~/${ACCOUNT3_DIR}` || base === ACCOUNT3_DIR) return "account3";
  return "custom";
}
...
export function resolveSlotDirectory(
  ...
  if (slot === "account3") {
    return homeDir ? joinHomeDir(homeDir, ACCOUNT3_DIR) : `~/${ACCOUNT3_DIR}`;
  }
