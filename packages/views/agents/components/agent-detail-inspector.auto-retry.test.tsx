...
describe("AgentDetailInspector auto retry", () => {
  afterEach(() => {
    cleanup();
  });

  it("treats a missing auto_retry_enabled field as on", () => {
    renderWithI18n(
    ...
    expect(screen.getByRole("switch", { name: "Auto retry" })).toBeChecked();
  });
  ...
  it("submits false when the switch is turned off", async () => {
    ...
    renderWithI18n(
      <AgentDetailInspector
        agent={agentFixture({ auto_retry_enabled: true })}
        runtime={null}
        runtimes={[]}
        members={[]}
        currentUserId="user-1"
  ...
  it("submits true when the switch is turned on from an explicit off state", async () => {
    const onUpdate = vi.fn(async () => {});
    const user = userEvent.setup();
    renderWithI18n(
      <AgentDetailInspector
        agent={agentFixture({ auto_retry_enabled: false })}
        runtime={null}
        runtimes={[]}
        members={[]}
        currentUserId="user-1"
